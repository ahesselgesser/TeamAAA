...
def table_two_access(is_accredited, column, table, row_iter, data, one_cols, max_rows):
    ...
    @param is_accredited - Boolean value, True if the report is for an accredited degree program.
...
def table_four_access(is_accredited, column, table, row_iter, data_coll_list, slo_count):
...
def run(uploaded_filename):
    ...
    ## Used to store the SLO Table information that we are interested in.
    ...
    for slo in range (0, slo_count):
        if (not is_undergrad and report_year == 19):
            slo_list.append(data[slo]['\nStudent Learning Outcomes'])
        else:
